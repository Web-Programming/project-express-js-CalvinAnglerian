        ...
        for (const item of orderItems){
            ...
            if(!product) {
                return res.status(404).json({
                    success: false,
                    message: `Product dengan ID ${item.Product} tidak ditemukan.`
                });
            }
...
// @desc    Mengambil Detail Pemesanan
const detail = async (req, res) => {
    try {
        ...
        .populate('orderItems.produc', 'nmae price'); //Ambil nama & harga dari produk
...
const update = async (req, res) => {
    ...
    try{
        const order = await Order.findByIdAndUpdate(re1.params.id, updateFields,{
            new: true,
            runValidators: true,
        });
        ...
        res.status(500).json({
            success: false,
            message: 'Kesalahan Server Interna. : l' + err.message
        });
    }
};

module.exports = {create, all, detail, update};